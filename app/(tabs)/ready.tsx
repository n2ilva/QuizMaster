...
const knownTrackStyles: Record<string, { icon: ComponentProps<typeof MaterialIcons>['name']; color: string }> = {
...
export default function ReadyCardsScreen() {
  ...
  const tracks: TrackCard[] = trackCatalog.map((item) => {
    const style = knownTrackStyles[item.key] ?? { icon: 'school', color: '#3F51B5' };
    return {
      key: item.key,
      label: item.label,
      icon: style.icon,
