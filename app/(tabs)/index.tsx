...
const knownThemeStyles: Record<string, { icon: ComponentProps<typeof MaterialIcons>['name']; color: string }> = {
...
const features: FeatureItem[] = [
  ...
  { icon: 'emoji-events', text: 'Sistema de medalhas (Bronze → Diamante)' },
  { icon: 'leaderboard', text: 'Ranking comunitário por medalha' },
  { icon: 'speed', text: 'Pontuação por performance e velocidade' },
  { icon: 'timer', text: 'Temporizador por questão em tempo real' },
  { icon: 'play-circle-filled', text: 'Continuação de lições interrompidas' },
  { icon: 'bar-chart', text: 'Progresso detalhado por categoria' },
  { icon: 'analytics', text: 'Estatísticas e porcentagem de estudo' },
];
...
export default function HomeScreen() {
  ...
  const themes: ThemeItem[] = trackCatalog.map((item) => {
    const style = knownThemeStyles[item.key] ?? { icon: 'school', color: '#3F51B5' };
    return {
      key: item.key,
      label: item.label,
      icon: style.icon,
