...
function getSLADuration(level: string, categoryId?: string): number {
  return (
    (SLA_DURATION_MAP[level] ?? SLA_DURATION_MAP.Baixa)[
      categoryId ?? "suporte_tecnico"
    ] ?? 45
  );
}
...
export function QuickResponseScreen() {
  ...
  const data =
    (quickResponseCatalog as QuickResponseData | null) ??
    EMPTY_QUICK_RESPONSE_DATA;

  const [selectedCategory, setSelectedCategory] =
    useState<QuickResponseCategory | null>(null);
  const [activeExercise, setActiveExercise] =
    useState<QuickResponseExercise | null>(null);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [feedback, setFeedback] = useState<{
    isCorrect: boolean;
    message: string;
    stats?: { attempts: number; timeBonus: number; withinSLA: boolean };
  } | null>(null);
  const [answeredStats, setAnsweredStats] = useState<
    Record<string, { attempts: number; withinSLA: boolean }>
  >({});
  ...
  const handleValidate = () => {
    ...
    const correctIds = activeExercise.actions
      .filter((a) => a.is_correct)
      .map((a) => a.id);
    const selectedList = Array.from(selectedIds);
    const isExactlyCorrect =
      selectedList.length === correctIds.length &&
    ...
    if (isExactlyCorrect) {
      ...
      setIsValidated(true);
      playSound("error");
      const wrongSelected = activeExercise.actions.find(
        (a) => selectedIds.has(a.id) && !a.is_correct,
      );
      const missingCorrect = activeExercise.actions.find(
        (a) => a.is_correct && !selectedIds.has(a.id),
      );
      ...
      if (wrongSelected?.feedback) msg = wrongSelected.feedback;
        ...
        msg = "Você esqueceu de uma ação crítica para a resolução.";
  ...
  const handleNext = () => {
    ...
    const currentIndex = selectedCategory.exercises.findIndex(
    ...
    >
      ...
      <View style={{ paddingHorizontal: 20, marginBottom: 24 }}>
        ...
        <Text style={{ fontSize: 15, color: textMuted, lineHeight: 22 }}>
          Encare incidentes reais de TI e prove que você é um especialista em
      ...
      <View style={[styles.maxContentWidth, { paddingHorizontal: 20 }]}>
        <View style={{ gap: 16 }}>
          {data.categories.map((cat) => {
            const completedCount = cat.exercises.filter(
              (ex) => answeredStats[ex.id],
            ).length;
                      ...
                      >
                        <View
                          style={{
                            flex: 1,
                            height: 4,
                            backgroundColor: isDark ? "#2D3139" : "#E2E8F0",
                            borderRadius: 2,
                          }}
                        >
                          <View
          ...
          {selectedCategory?.exercises
            .filter(
              (ex) =>
                ...
                ex.level.toUpperCase() === priorityFilter,
            )
            .sort((a, b) => {
                    ...
                    >
                      ...
                      >
                        {isAnswered ? (
                          ...
                          <Text style={{ fontWeight: "800", color: textMuted }}>
                            {index + 1}
                          </Text>
                        )}
                      </View>
                              ...
                              >
                                ...
                                <Text
                                  style={{
                                    fontSize: 11,
                                    fontWeight: "700",
                                    color: stats.withinSLA
                                      ? "#22C55E"
                                      : "#EF4444",
                                  }}
                                >
                                  {stats.withinSLA ? "SLA OK" : "SLA OUT"}
                                </Text>
                              </View>
                              <View
                                style={{
        ...
        >
          ...
          >
            ...
            {(() => {
              ...
              const pColor = PRIORITY_COLORS[pLabel];
              const cardBg = isDark
                ? PRIORITY_BG_DARK[pLabel]
                : PRIORITY_BG_LIGHT[pLabel];
              const pTitle =
                pLabel === "P1"
                  ? "INCIDENTE P1 — CRÍTICO"
                  : pLabel === "P2"
                    ? "INCIDENTE P2 — URGENTE"
                    : "INCIDENTE P3 — BAIXA";
          ...
          <View style={[styles.maxContentWidth, { paddingHorizontal: 20 }]}>
            ...
            {activeExercise.context && (
              ...
              >
                ...
                >
                  ...
                  <MaterialIcons
                    name={showContextCard ? "expand-less" : "expand-more"}
                    size={20}
                    color={textMuted}
                  />
                </TouchableOpacity>
                  ...
                  >
                    ...
                    {activeExercise.context.log_snippet && (
                      <View style={{ marginTop: 4 }}>
                        ...
                        <View
                          style={{
                            backgroundColor: isDark ? "#0D0F10" : "#F8FAFC",
                            borderRadius: 8,
                            padding: 12,
                            borderWidth: 1,
                            borderColor: isDark
            ...
            >
              <View>
                ...
                >
                  ESTE INCIDENTE REQUER{" "}
                  {activeExercise.actions.filter((a) => a.is_correct).length}{" "}
                  AÇÕES CORRETAS
                </Text>
              </View>
              <Text
    ...
    <View style={{ flex: 1, backgroundColor: bg, paddingTop: topPadding }}>
      ...
      >
        <View
          style={{
            flex: 1,
            backgroundColor: isSmallScreen ? bg : "rgba(0,0,0,0.88)",
            padding: 0,
          }}
        >
          <ScrollView
            ...
            contentContainerStyle={{ 
              flexGrow: 1, 
              paddingTop: isSmallScreen ? insets.top : 20,
              paddingBottom: isSmallScreen ? insets.bottom : 20,
          ...
          >
            <PanelCard
              style={{
                ...
                borderColor: feedback?.isCorrect ? "#22C55E40" : "#EF444440",
                shadowColor: "#000",
                shadowOffset: { width: 0, height: 20 },
                shadowOpacity: 0.5,
                shadowRadius: 30,
                elevation: isSmallScreen ? 0 : 24,
                width: isSmallScreen ? "100%" : 520,
                minHeight: isSmallScreen ? "100%" : undefined,
              }}
            >
              {/* Header */}
              <View style={{ alignItems: "center", marginBottom: 20 }}>
                <View
                  style={{
                    ...
                    justifyContent: "center",
                    marginBottom: 12,
                    shadowColor: feedback?.isCorrect ? "#22C55E" : "#EF4444",
                ...
                >
                  <MaterialIcons
                    name={feedback?.isCorrect ? "verified" : "report-problem"}
                    size={40}
                    color="#FFFFFF"
                  />
                </View>
              ...
              <View style={{ gap: 12, marginBottom: 20 }}>
                  ...
                  <View style={{ flexDirection: "row", gap: 10 }}>
                    ...
                    >
                      <MaterialIcons
                        name="timer"
                        size={16}
                        color={feedback.stats.withinSLA ? "#22C55E" : "#EF4444"}
                      />
                      <Text
                        style={{
                          fontSize: 9,
                      ...
                      <Text
                        style={{
                          fontSize: 13,
                          fontWeight: "900",
                          color: feedback.stats.withinSLA
                            ? "#22C55E"
                            : "#EF4444",
                        }}
                      >
                        {feedback.stats.withinSLA ? "OK" : "OUT"}
                      </Text>
                    </View>
                    <View
                      style={{
                    ...
                    >
                      ...
                      >
                        {
                          activeExercise?.actions.filter((a) => a.is_correct)
                            .length
                        }
                      </Text>
                    </View>
                        ...
                        >
                          <View
                            style={{
                              ...
                              height: 20,
                              borderRadius: 10,
                              backgroundColor: action.is_correct
                                ? "#22C55E20"
                                : "#EF444420",
                              alignItems: "center",
                              justifyContent: "center",
                          ...
                          >
                            <MaterialIcons
                              name={action.is_correct ? "check" : "close"}
                              size={12}
                              color={action.is_correct ? "#22C55E" : "#EF4444"}
                            />
                          </View>
                          <View style={{ flex: 1 }}>
                            <Text
                              style={{
                                fontSize: 13,
                                color: action.is_correct
                                  ? textPrimary
                                  : textMuted,
                                fontWeight: action.is_correct ? "600" : "400",
                                lineHeight: 18,
                              }}
                            >
                              {action.text}
                ...
                {activeExercise?.runbook_steps &&
                  activeExercise.runbook_steps.length > 0 && (
                    <View
                      style={{
                        backgroundColor: isDark
                ...
                {feedback?.isCorrect ? (
                  ...
                  <View style={{ flexDirection: "row", gap: 10 }}>
                    <TouchableOpacity
                      ...
                      style={{
                        flex: 1,
                        backgroundColor: isDark ? "#2D3139" : "#F1F5F9",
                        paddingVertical: 14,
                        borderRadius: 14,
                        alignItems: "center",
                        borderWidth: 1,
