...
function CategoryCard({
  ...
  const currentDifficulty = stats?.currentDifficulty;
  const currentDifficultyMasteryPercent = stats?.currentDifficultyMasteryPercent ?? 0;
  const nextDifficulty = stats?.nextDifficulty;
  ...
  return (
    <View className="rounded-2xl border border-[#E6E8EB] bg-[#FAFBFC] p-4 dark:border-[#30363D] dark:bg-[#1A1D21] md:w-[48.5%]">
      ...
      {currentDifficulty && (
        <View className="mt-3 rounded-xl bg-[#EEF2FF] px-3 py-2 dark:bg-[#1E293B]">
          <Text className="text-[11px] font-semibold text-[#3F51B5] dark:text-[#A5B4FC]">
            Nível atual: {currentDifficulty}
          </Text>
          <Text className="mt-0.5 text-[11px] text-[#687076] dark:text-[#9BA1A6]">
            {nextDifficulty
              ? `${currentDifficultyMasteryPercent}% dominado. Libera ${nextDifficulty} com 100% neste nível.`
              : 'Todos os níveis desta categoria já estão liberados.'}
          </Text>
        </View>
      )}
    ...
    <Link
      ...
      <Pressable
        ...
        <View style={{ paddingVertical: 18, paddingLeft: 24, paddingRight: 20 }}>
          ...
          {currentDifficulty && (
            <View style={{ backgroundColor: '#151B2E', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 12 }}>
              <Text style={{ color: '#A5B4FC', fontSize: 11, fontWeight: '700' }}>
                Nível atual: {currentDifficulty}
              </Text>
              <Text style={{ color: '#9BA1A6', fontSize: 11, marginTop: 3 }}>
                {nextDifficulty
                  ? `${currentDifficultyMasteryPercent}% dominado. Libera ${nextDifficulty} com 100% neste nível.`
                  : 'Todos os níveis desta categoria já estão liberados.'}
              </Text>
      ...
      <View style={{ flex: 1, flexDirection: 'row', backgroundColor: '#0D0F10' }}>
        ...
        <ScrollView
          ...
          <Pressable
            ...
            <Text style={{ color: '#687076', fontSize: 14 }}>Voltar para Quiz</Text>
          </Pressable>
